...
from panda_tools import panda_merge#, make_forest, use_forest
import pandas as pd
...
def load_table(path):
    ...
    table = table[['date', 'hometeam', 'awayteam', 'fthg', 'ftag', 'ftr', 'hthg', 'htag', 'htr', 'hr', 'ar', 'whh', 'whd', 'wha']]
...
def make_table_team(table):
    # Team home
    table_team_h = table[['date', 'hometeam', 'fthg', 'hthg', 'hr', 'ftr']]
    ...
    table_team_h.columns = ['date', 'team', 'ftg', 'htg', 'r', 'win', 'draw', 'lose', 'h_or_a']
    
    # Team away
    table_team_a = table[['date', 'awayteam', 'ftag', 'htag', 'ar', 'ftr']]
    ...
    table_team_a.columns = ['date', 'team', 'ftg', 'htg', 'r', 'win', 'draw', 'lose', 'h_or_a']
...
def make_stats_table(table_team, five = 10):
    ...
    for metric in [ftg_last_five, win_last_five, draw_last_five, red_last, ftg_per_match, num_matchs_in_last_2_weeks]:
        metric = metric.reset_index()
        stats_table = stats_table.merge(metric, on = ['team', 'level_1'], how = 'outer')
        
    stats_table.columns = ['team', 'index', 'ftg_last_five', 'win_last_five', 'draw_last_five', 'red_last', 'ftg_per_match', 'num_matchs_in_last_2_weeks']
